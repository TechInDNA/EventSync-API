...
class PutSessionServiceTest {
    ...
    private static final String SESSION_TITLE = "Updated Session Title";

    PutSessionServiceTest() {
    ...
    private SessionUpdateInputDto validRequest() {
        return SessionUpdateInputDto.builder()
                .title(SESSION_TITLE)
    ...
    @Test
    @DisplayName("with valid mandatory fields returns updated session")
    void withValidData_returnsUpdatedSession() {
        ...
        when(sessionRepository.updateSessionById(
                eq(SESSION_ID), eq(SESSION_TITLE), eq("An updated description"),
                eq(Instant.parse("2025-07-01T09:00:00Z")),
                eq(Instant.parse("2025-07-01T10:00:00Z")),
                eq(ROOM_ID), eq(200), eq(EVENT_ID)
        ...
        var result = sessionService.updateSession(SESSION_ID, request);
        ...
        verify(sessionRepository).updateSessionById(
                eq(SESSION_ID), eq(SESSION_TITLE), eq("An updated description"),
                eq(Instant.parse("2025-07-01T09:00:00Z")),
                eq(Instant.parse("2025-07-01T10:00:00Z")),
                eq(ROOM_ID), eq(200), eq(EVENT_ID)
    ...
    @Test
    @DisplayName("with unknown id throws NotFoundException")
    void withUnknownId_throwsNotFound() {
        var request = validRequest();
        ...
        assertEquals("Session " + SESSION_ID + " not found.", exception.getMessage());
    }
    ...
    void withDuplicateTitle_throwsConflict() {
        ...
        when(sessionRepository.updateSessionById(
                ...
                "duplicate key", new SQLException("", "23505", 23505))
        );

        var exception = assertThrows(ConflictException.class,
                () -> sessionService.updateSession(SESSION_ID, request));
        assertEquals("Session '" + SESSION_TITLE + "' already exists.", exception.getMessage());
    }

    @Test
    @DisplayName("with non-existent room throws NotFoundException")
    void withNonExistentRoom_throwsNotFound() {
        var request = validRequest();
